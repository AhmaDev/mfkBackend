...
router.get(`/recievers/:locationId`, function (req, res, next) {
  connection.query(
    `SELECT receiver FROM ${tableName} WHERE locationId = ${req.params.locationId} GROUP BY receiver`,
    (err, result) => {
      result = result.filter(
        (e) =>
          e.receiver != "تدوير مبلغ تلقائي" &&
          e.receiver != null &&
          e.receiver != "",
      );
      res.send(result);
